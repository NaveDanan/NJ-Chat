...
import { ChevronsLeft, ChevronsRight, Folder, LogOut, Pencil, Plus, Search, Settings, Star } from "lucide-react";
...
type Message = { id?: string; role: "user" | "assistant"; content: string; model?: string; usage?: any };
...
export default function ChatLayout() {
  ...
  async function refreshModels() {
    try {
      ...
      setModels(r.models || []);
      if (!model && (me?.settings?.model || r.models?.[0]?.id)) {
        setModel(me?.settings?.model || r.models?.[0]?.id || "");
      }
    } catch {}
  }
  ...
  async function refreshChats(openFirst = false) {
    ...
    setChats(r.chats || []);
    if (openFirst && r.chats?.length) {
      openChat(r.chats[0].id);
    } else if (openFirst && !r.chats?.length) {
      await newChat();
    }
  }
        ...
        if (eventName && dataBuf.length) {
          ...
          try {
            ...
            if (eventName === "ack") {
              ...
              setThinking((t) => t + (payload.content || ""));
  ...
  async function editMessage(target: Message) {
    ...
    try {
      await api(`/api/chats/${chatId}/messages/${target.id}`, { 
        method: "PATCH", 
    ...
    <div className="grid h-screen grid-cols-1 md:grid-cols-[var(--sidebar)_1fr]" style={{ ['--sidebar' as any]: `${collapsed ? 64 : sidebarW}px` }}>
                      ...
                      {!collapsed && (
                        <>
                          ...
                          <button 
                            ...
                            onClick={async (e) => { 
                              e.stopPropagation(); 
                              const f = prompt("Move to folder (blank to unset)", c.folder || ""); 
                              if (f !== null) { 
                                await api(`/api/chats/${c.id}`, { 
                                  method: "PATCH", 
                                  body: JSON.stringify({ folder: f.trim() }) 
                                }); 
                                await refreshChats(); 
                              } 
                            }}
      ...
      <main className="flex min-h-0 flex-col">
        <header className="flex flex-wrap items-center gap-2 border-b border-border bg-secondary/30 px-3 py-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={stop} disabled={!streaming}>Stop</Button>
            <Button variant="outline" size="sm" onClick={async () => { 
              if (!chatId) return; 
              const r = await api(`/api/chats/${chatId}`); 
              setSystemText(r?.chat?.system || ""); 
              setShowSystem(true); 
            }}>System</Button>
        ...
        <section id="messages" className="flex-1 space-y-3 overflow-auto p-4" style={{ background: gradientBg(gradient) }}>
          {messages.map((m, i) => (
            <MessageItem key={i} m={m as any} onRegenerate={regenerateFromMessage} onEdit={editMessage} isStreaming={streaming} />
          ))}
          {!!thinking && (
            <div className="mx-auto w-full max-w-3xl rounded-xl border border-dashed border-border bg-background/40 p-2 text-[13px] text-muted-foreground">
              Thinking… {thinking}
            </div>
          )}
        </section>
        ...
        <footer className="border-t border-border bg-background/60 p-3">
          <div className="mx-auto flex w-full max-w-3xl items-start gap-2">
            ...
            />
            <Button onClick={send} disabled={streaming || !content.trim()}>Send</Button>
          </div>
        </footer>
      </main>
      ...
      <SettingsDrawer 
        ...
        onOpenChange={(o) => { 
          setShowSettings(o); 
          if (!o) { 
            setGradient(me?.settings?.gradient || gradient); 
          } 
        }} 
        initial={me?.settings} 
      />
            ...
            <div className="mt-3 text-right">
              ...
              <Button onClick={async () => { 
                ...
                await api(`/api/chats/${chatId}`, { 
                  method: "PATCH", 
                  body: JSON.stringify({ system: systemText }) 
                }); 
                setShowSystem(false); 
              }}>Save</Button>
            </div>
